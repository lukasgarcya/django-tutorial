	...
	\section{Instalação}	
		...
		\subsection{Pip}
			...
			\begin{frame}[fragile]{Instalando o pip}
				...
				Este arquivo nada mais é do que comandos em Python que irão baixar a última versão do pip e instalar para nós.
		...
		\subsection{VirtualEnvWrapper}
			\begin{frame}[fragile]{Instalando o VirtualEnvWrapper}
				De acordo com o site oficial \url{http://virtualenvwrapper.readthedocs.org/en/latest/install.html} a instalação consiste em utilizar o pip instalado anteriormente com o seguinte comando.
				\begin{lstlisting}
	...
	\section{Iniciando no Django}
		...
		\subsection{Criando projeto inicial}
			...
			\begin{frame}[fragile]{Realizando as configurações básicas no projeto}{Linguagem}
				Por padrão o Django vem configurado para funcionar na linguagem inglês do Estados Unidos. Então agora vamos colocar em Português do Brasil alterando a variável LANGUAGE\_CODE para 'pt-br'.
				\begin{lstlisting}
				LANGUAGE_CODE = 'pt-br'
				\end{lstlisting}
			\end{frame}
	...
	\section{Criando a app de Movimentação}
		...
		\subsection{Criando uma Transação}
			...
			\begin{frame}[fragile]{Instalando a app no projeto}
				Mesmo após criar nossa app ainda é necessário que seja informado no settings.py que esta app irá ser utilizada no projeto. Para isto basta adicionar a app na lista INSTALLED\_APPS como segue abaixo:
				\begin{lstlisting}
				INSTALLED_APPS = [
		...
		\subsection{Criando a migração}	
			...
			\begin{frame}[fragile]{Fazendo a migração do banco de dados}{Criando o banco de dados}
				Após o makemigrations é necessário aplicar as alterações no banco de dados e isto é feito através do migrate. Então agora vamos utilizar o migrate e ver que ele já vai criar o banco de dados e vai ter a estrutura do modelo Transacao e Categoria.
				\begin{lstlisting}
		...
		\subsection{Django Contrib Admin}
			...
			\begin{frame}[fragile]{Conhecendo o admin}
				Vamos clicar em Usuário e veremos uma lista de usuários que estão cadastrados e neste caso tem-se apenas o admin. Para editar o usuário basta clicar no link do nome do admin e irá aparecer a edição. Nesta pode-se alterar o nome do usuário, informações pessoais, as permissões e datas importantes.
			\end{frame}
